...
interface PixelShiftModalProps {
  error: string | null;
  ...
  onMerge(method: string, motionCompensation: boolean): void;
  onMethodChange(method: string): void;
  onMotionCompensationChange(enabled: boolean): void;
  progressMessage: string | null;
}

export default function PixelShiftModal({
  error,
  finalImageBase64,
  frameCount,
  isOpen,
  ...
  onMerge,
  onMethodChange,
  onMotionCompensationChange,
  progressMessage,
}: PixelShiftModalProps) {
  ...
  const handleStartMerge = () => {
    onMerge(mergeMethod, motionCompensation);
  };
  ...
  const renderContent = () => {
    ...
    return (
      <div className="flex flex-col items-center justify-center py-6 h-[460px]">
        ...
        <div className="w-full max-w-sm space-y-3 mt-2">
          <div>
            ...
            >
              <option value="drizzle">Bayer Drizzle (Super-Res)</option>
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              ...
              className={`relative w-10 h-5 rounded-full transition-colors ${
                motionCompensation ? 'bg-accent' : 'bg-surface'
              }`}
            >
              <div
                className={`absolute top-0.5 w-4 h-4 rounded-full bg-white transition-transform ${
                  motionCompensation ? 'left-5' : 'left-0.5'
                }`}
              />
            </button>
          </div>
        ...
        <div className="flex flex-col">
          ...
          <div
            className={`mt-4 flex justify-end gap-3 ${
              savedPath ? '' : 'pt-4 border-t border-surface/50'
            }`}
          >
            {renderButtons()}
          </div>
